...
class TestDeleteMultipleObjects(unittest.TestCase):
    ...
    def test_serialize_delete_multiple_objects_request_error_conditions(self):
        ...
        self.assertIn('missing required field, request.objects', str(context.exception))

        # Test serialization error when new parameters objects is not set
        request = model.DeleteMultipleObjectsRequest(
            bucket='bucket_name',
            delete=model.Delete(
                quiet=True,
        ...
        self.assertIn('missing required field, delete.objects', str(context.exception))
