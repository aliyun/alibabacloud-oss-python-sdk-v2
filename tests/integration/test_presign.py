# pylint: skip-file
"""Integration tests for presign functionality."""
import datetime
import requests
...
class TestPresign(TestIntegration):
    ...
    def test_presign_get_object_v1(self):
        ...
        pre_result = self.signv1_client.presign(
        ...
        # Actually download using the presigned URL
        with requests.get(pre_result.url, headers=pre_result.signed_headers) as resp:
            if resp.status_code != 200:
                self.fail(f'GET failed: status={resp.status_code}, body={resp.text}, '
    ...
    def test_presign_put_object_v1(self):
    ...
    def test_presign_put_object_with_content_type_v1(self):
        ...
        # Actually upload using the presigned URL with signed headers
        with requests.put(pre_result.url, headers=pre_result.signed_headers, data=data) as resp:
            if resp.status_code != 200:
                self.fail(f'PUT with Content-Type failed: status={resp.status_code}, body={resp.text}, '
                          f'signed_headers={pre_result.signed_headers}')

        # Verify uploaded content and content type
        head_result = self.signv1_client.head_object(oss.HeadObjectRequest(
            bucket=self.bucket_name,
            key=key))
    ...
    def test_presign_head_object_v1(self):
        ...
        # Actually perform HEAD request using the presigned URL
        with requests.head(pre_result.url, headers=pre_result.signed_headers) as resp:
            if resp.status_code != 200:
                self.fail(f'HEAD failed: status={resp.status_code}, body={resp.text}')
    ...
    def test_presign_get_object_v4(self):
        ...
        with requests.get(pre_result.url, headers=pre_result.signed_headers) as resp:
            if resp.status_code != 200:
                self.fail(f'GET v4 failed: status={resp.status_code}, body={resp.text}')
            self.assertEqual(data.encode(), resp.content)

    def test_presign_put_object_v4(self):
        ...
        with requests.put(pre_result.url, headers=pre_result.signed_headers, data=data) as resp:
            if resp.status_code != 200:
                self.fail(f'PUT v4 failed: status={resp.status_code}, body={resp.text}, '
                          f'signed_headers={pre_result.signed_headers}')

        # Verify uploaded content
        get_result = self.client.get_object(oss.GetObjectRequest(
            bucket=self.bucket_name,
            key=key))
        self.assertEqual(200, get_result.status_code)
        self.assertEqual(data.encode(), get_result.body.content)

    def test_presign_multipart_upload_v1(self):
        ...
        init_pre = self.signv1_client.presign(
        ...
        with requests.post(init_pre.url, headers=init_pre.signed_headers) as resp:
            if resp.status_code != 200:
                self.fail(f'InitiateMultipartUpload failed: status={resp.status_code}, body={resp.text}')
            self.assertEqual(200, resp.status_code)
            init_result = oss.InitiateMultipartUploadResult()
            oss.serde.deserialize_xml(xml_data=resp.content, obj=init_result)
            upload_id = init_result.upload_id
            self.assertIsNotNone(upload_id)

        # Step 2: Presign UploadPart (PUT)
        part_pre = self.signv1_client.presign(
        ...
        with requests.put(part_pre.url, headers=part_pre.signed_headers, data=part_data) as resp:
            if resp.status_code != 200:
                self.fail(f'UploadPart failed: status={resp.status_code}, body={resp.text}')
            self.assertEqual(200, resp.status_code)
        ...
        complete_pre = self.signv1_client.presign(complete_request, expiration=expiration)
        self.assertEqual('POST', complete_pre.method)

        with requests.post(complete_pre.url, headers=complete_pre.signed_headers, data=op_input.body) as resp:
            if resp.status_code != 200:
                self.fail(f'CompleteMultipartUpload failed: status={resp.status_code}, body={resp.text}')
            self.assertEqual(200, resp.status_code)
    ...
    def test_presign_abort_multipart_upload_v1(self):
        ...
        init_result = self.signv1_client.initiate_multipart_upload(
            oss.InitiateMultipartUploadRequest(
                bucket=self.bucket_name,
                key=key))
        ...
        abort_pre = self.signv1_client.presign(
        ...
        with requests.delete(abort_pre.url, headers=abort_pre.signed_headers) as resp:
            if resp.status_code != 204:
                self.fail(f'AbortMultipartUpload failed: status={resp.status_code}, body={resp.text}')
            self.assertEqual(204, resp.status_code)

        # Step 4: Verify upload is aborted by listing multipart uploads
        list_result = self.signv1_client.list_multipart_uploads(
    ...
    def test_presign_get_object_fail(self):
