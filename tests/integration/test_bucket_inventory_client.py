...
class TestBucketInventory(TestIntegration):
    ...
    def test_bucket_inventory_with_incremental(self):
        # create bucket
        bucket_name = random_bucket_name()
        result = self.client.put_bucket(oss.PutBucketRequest(
            bucket=bucket_name,
        ...
        inventory_id = 'test-oss-' + random_lowstr(6)
        try:
            # put bucket inventory with incremental inventory
            result = self.client.put_bucket_inventory(oss.PutBucketInventoryRequest(
                bucket=bucket_name,
                inventory_id=inventory_id,
                inventory_configuration=oss.InventoryConfiguration(
            ...
            self.assertEqual(1637883649, result.inventory_configuration.filter.last_modify_begin_time_stamp)
            self.assertEqual(1638347592, result.inventory_configuration.filter.last_modify_end_time_stamp)
            self.assertEqual(True, result.inventory_configuration.incremental_inventory.is_enabled)
            self.assertEqual(600, result.inventory_configuration.incremental_inventory.schedule.frequency)
            fields = result.inventory_configuration.incremental_inventory.optional_fields.fields
            expected_fields = [
                oss.IncrementalInventoryOptionalFieldType.SEQUENCE_NUMBER,
                oss.IncrementalInventoryOptionalFieldType.RECORD_TYPE,
                oss.IncrementalInventoryOptionalFieldType.RECORD_TIMESTAMP,
            ...
            self.assertEqual(len(expected_fields), len(fields))
        except Exception as e:
            ope = cast(oss.exceptions.OperationError, e)
            self.assertIsInstance(ope.unwrap(), oss.exceptions.ServiceError)
            serr = cast(oss.exceptions.ServiceError, ope.unwrap())
            self.assertEqual(403, serr.status_code)
            self.assertEqual(24, len(serr.request_id))
            self.assertEqual('AccessDenied', serr.code)
