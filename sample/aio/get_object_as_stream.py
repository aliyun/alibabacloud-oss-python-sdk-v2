...
parser = argparse.ArgumentParser(description="get object as stream sample")
...
async def main():
    ...
    result = await client.get_object_as_stream(oss.GetObjectRequest(
        bucket=args.bucket,
        key=args.key,
    ))
    ...
    # The data is not buffered in memory, and a broken connection is resumed
    # from the last consumed offset.
