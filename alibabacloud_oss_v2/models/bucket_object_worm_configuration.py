...
class PutBucketObjectWormConfigurationRequest(serde.RequestModel):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        Raises:
            ValueError: If object_worm_configuration rule validation fails
        """
        super().__init__(**kwargs)
        
        # Validation: if rule is set, validate days and years
        if object_worm_configuration and object_worm_configuration.rule:
            rule = object_worm_configuration.rule
            if rule.default_retention:
                retention = rule.default_retention
                
                # Validation: days and years cannot both be None
                if retention.days is None and retention.years is None:
                    raise ValueError("Either 'days' or 'years' must be specified in rule.default_retention")
                
                # Validation: if days is specified, it must be > 0
                if retention.days is not None and retention.days <= 0:
                    raise ValueError("'days' must be greater than 0 when specified")
                
                # Validation: if years is specified, it must be > 0
                if retention.years is not None and retention.years <= 0:
                    raise ValueError("'years' must be greater than 0 when specified")
