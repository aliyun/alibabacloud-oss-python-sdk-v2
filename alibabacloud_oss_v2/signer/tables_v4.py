# -*- coding: utf-8 -*-
"""Tables V4 signature implementation"""
...
class TablesSignerV4(Signer):
    ...
    def _calc_canonical_request(self,
        ...
        # canonical uri - Tables-specific format
        # without bucket arn: acs:osstables:cn-hangzhou:{}:bucket/{}/${urldecode(path)}
        # with    bucket arn: ${bucket-arn}/${urldecode(path)}
        uri_parts = []
        uri_parts.append('/')
        if signing_ctx.bucket is not None and signing_ctx.bucket != '':
            uri_parts.append(signing_ctx.bucket)
            uri_parts.append('/')
        else:
            region = signing_ctx.region or ''
            uri_parts.append(f'acs:osstables:{region}::bucket/')
            uri_parts.append('/')

        if signing_ctx.key is not None and signing_ctx.key != '':
            uri_parts.append(unquote(signing_ctx.key))

        canonical_uri = quote(''.join(uri_parts), safe='/')
