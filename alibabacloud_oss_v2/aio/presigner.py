"""Async APIs for presign operation."""
# pylint: disable=line-too-long
import datetime
from typing import Union, Optional, MutableMapping, Any
from ..types import OperationInput, OperationOutput, AsyncHttpClient, HttpRequest, AsyncHttpResponse
from .. import serde
from .. import models
...
class _nopAsyncHttpResponse(AsyncHttpResponse):
    ...
    def __repr__(self) -> str:
        return '_nopAsyncHttpResponse'

    async def __aenter__(self) -> "_nopAsyncHttpResponse":
        return self
...
class _nopAsyncHttpClient(AsyncHttpClient):
    async def send(self, request: HttpRequest, **kwargs: Any) -> AsyncHttpResponse:
        return _nopAsyncHttpResponse(request=request)
...
_presign_kwargs = {
    'http_client': _nopAsyncHttpClient(),
    'auth_method': 'query'
}


async def presign_inner(client: _AsyncClientImpl, request: PresignRequest, **kwargs) -> PresignResult:
    """
    presign asynchronously
    ...
    op_output = await client.invoke_operation(op_input, **_presign_kwargs)
...
def _deserialize_output(client: _AsyncClientImpl, op_output: OperationOutput) -> PresignResult:
