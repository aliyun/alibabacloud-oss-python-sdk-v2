...
|**kwargs|Any|(可选)接口级别的配置参数，例如重试次数

**返回值列表**：

|返回值名|类型|说明
|:-------|:-------|:-------
|result|*\<OperationName\>Result|请求成功时，返回结果值；请求失败时，抛错误异常

**kwargs选项**：

|参数名|类型|说明
|:-------|:-------|:-------
|retry_max_attempts|int|HTTP请求时的最大尝试次数
|readwrite_timeout|int| 应用读写数据的超时时间
...
阈值(上传/下载/拷贝) 表示 对象/文件 大小 大于该值时，使用分片方式(上传/下载/拷贝)。
